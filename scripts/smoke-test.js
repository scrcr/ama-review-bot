...
assert(
  schema.includes("CONSTRAINT uniq_products_region_asin UNIQUE (region, asin)"),
  "Products table should scope ASIN uniqueness per region"
);
assert(
  schema.includes("CHECK (status IN ('pending', 'posted', 'failed'))"),
  "Post queue status should be constrained to expected values"
);
assert(
  schema.includes("CREATE TABLE IF NOT EXISTS posted_tweets"),
  "Schema must include posted_tweets table"
);
assert(
  schema.includes("uniq_posted_tweets_region_day_asin"),
  "Posted tweets should enforce region/day ASIN uniqueness"
);
