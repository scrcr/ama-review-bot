...
function getJstDayRange(date = new Date()) {
  const jst = toJst(date);
...
async function getTweetHistory(currentDate) {
  ...
  let lastGlobalPost = await getMostRecentPosted();
  if (lastGlobalPost) {
    const config = loadRegionConfig(lastGlobalPost.region);
    lastGlobalPost = {
      ...lastGlobalPost,
      url: buildAffiliateUrl(config, lastGlobalPost.asin),
    };
  }
  ...
  await Promise.all(
    regions.map(async (region) => {
      ...
      if (record) {
        ...
        lastPostByRegion.set(region, {
          ...record,
          url: buildAffiliateUrl(config, record.asin),
        });
      }
    })
  );
  ...
  for (const product of candidates) {
    ...
    for (const review of shuffledReviews) {
      try {
        ...
        const url = buildAffiliateUrl(regionConfig, product.asin);
        ...
        if (state.lastGlobalPost && state.lastGlobalPost.url === url) {
        ...
        await postTweet(region, { text, imageUrl: product.imageUrl });
        const postedAt = new Date();
        await recordPostedTweet(region, product.asin, text, postedAt);
  ...
  for (const region of regions) {
    try {
      ...
      if (outcome) {
        ...
        state.lastPostByRegion.set(region, { asin: outcome.asin, postText: outcome.text, url: outcome.url });
